...
const createOrUpdateConnectionData = async function (
  connection_id,
  key,
  value,
) {
  try {
    ...
    console.log('Connection form data saved successfully.')
    return true
  } catch (error) {
    console.error('Error saving connection form data to the database: ', error)
  }
}

const readConnectionStates = async function (connection_id) {
  try {
...
const readConnectionStatesByKey = async function (connection_id, key) {
  try {
    const connectionStates = await ConnectionsStates.findAll({
      where: {
        connection_id,
...
const readConnectionData = async function (connection_id) {
  try {
    const connectionData = await ConnectionsStates.findAll({
      where: {
        connection_id,
        key: 'credential_data',
      },
...
module.exports = {
  ...
  createOrUpdateConnectionData,
  readConnectionStates,
  readConnectionStatesByKey,
  readConnectionData,
